...
def calcstruct(inf,nbinsmax=20):
  """
  ...
  for k in range(nbinsmax):
    ...
    if ((dmxsq[ind]).size != 0):
      nbincount += len(dmxsq[ind])
      structmean     = np.mean(dmxsq[ind]) 
      #if (structmean > 0.):
      struct.append(np.mean(dmxsq[ind]))
      bins.append(np.mean([binedge[k],binedge[k+1]]))
      #structerr.append(np.std(dmxsq[ind])/np.sqrt((dmxsq[ind]).size))
      structerr.append(np.sqrt(np.sum((dmxsqerr[ind])**2))/np.float(dmxsq[ind].size))
