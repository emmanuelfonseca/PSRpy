...
from ..const import d2r, spy, G, c, M_sun, au
...
def m2sini(fdots,m1,ecc,omg,truea,truead):
    ...
    Required arguments:
        ...
        - 'm1'     = mass of pulsar (or inner-orbit objects, for a triple system; in M_sun).
        - 'ecc'    = eccentricity.
    ...
    A   = 1. + ecc*cos(truea)
    m1 *= M_sun 
    return -fdots[1]*c/fdots[0]/sin(truea+omg)*((m1**2)*(A**2)/G/truead**4)**(1./3.)/M_sun
...
def semimajor2(fdots,m1,ecc,omg,truea,truead):
    ...
    Required arguments:
        ...
        - 'truead' = time-derivative of true anomaly (in degrees.)
    """
    m2s = m2sini(fdots,m1,ecc,omg,truea,truead)*M_sun
    # convert input to have necessary units.
    omg, truea = omg*d2r, truea*d2r
    truead = truead/spy
